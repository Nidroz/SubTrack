...
export default function Search() {
    ...
    // load user list on mount to know what's already tracked
    useEffect(() => {
        fetchList()
    }, [])
    ...
    const getEntryId = (malId: number) =>
        entries.find(e => e.mediaId === malId)?.id
    ...
    const handleRemove = async (malId: number) => {
        ...
        if (entryId) await removeEntry(entryId)
    }
